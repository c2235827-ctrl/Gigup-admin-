...
export default function OrdersView({
  ...
  const [selectedStatus, setSelectedStatus] = useState(
    initialPendingFilter || initialBonusPendingFilter ? "pending" : "all",
  );
  ...
  const handleRequery = async (e: React.MouseEvent, order: Order) => {
    ...
    if (!order.smedata_ref) {
      addToast(
        "error",
        'No SMEDATA reference found. Use "Resend Bonus" for bonus orders or contact support for paid orders.',
      );
      return;
    }
    ...
    try {
      ...
      if (response.success) {
        if (response.smedata_status === "success") {
          ...
          addAuditLog(
            "order",
            "requery_transaction",
            `Successfully requeried order ${order.plan_name} for recipient ${order.recipient_phone} (Ref: ${ref}) - Status: SUCCESS`,
            "success",
          );
        } else if (response.smedata_status === "failed") {
          addToast(
            "warning",
            `❌ Order marked as FAILED - refunded (Ref: ${ref})`,
          );
          addAuditLog(
            "order",
            "requery_transaction",
            `Successfully requeried order ${order.plan_name} for recipient ${order.recipient_phone} (Ref: ${ref}) - Status: FAILED (Refunded)`,
            "success",
          );
        } else {
          addToast(
            "info",
            `Order returned status: ${response.smedata_status.toUpperCase()}`,
          );
          addAuditLog(
            "order",
            "requery_transaction",
            `Successfully requeried order ${order.plan_name} for recipient ${order.recipient_phone} (Ref: ${ref}) - Status: ${response.smedata_status.toUpperCase()}`,
            "success",
          );
        }
        ...
        addAuditLog(
          "order",
          "requery_transaction",
          `Failed to requery order ${order.plan_name} for recipient ${order.recipient_phone} (Ref: ${ref}) - API reported failure`,
          "failed",
        );
      }
    } catch (err: any) {
      ...
      addAuditLog(
        "order",
        "requery_transaction",
        `Failed to requery order (Ref: ${ref}): ${err.message || err}`,
        "failed",
      );
    } finally {
  ...
  const handleResendBonusFromOrder = async (
    e: React.MouseEvent,
    order: Order,
  ) => {
    e.stopPropagation();
    ...
    try {
      ...
      if (result.success) {
        addAuditLog(
          "order",
          "resend_signup_bonus",
          `Resent 1GB welcome data bonus to recipient ${order.recipient_phone} from order journal action. Ref: ${order.id}`,
          "success",
        );
        loadOrders();
      } else {
        addAuditLog(
          "order",
          "resend_signup_bonus",
          `Failed to resend welcome data bonus to recipient ${order.recipient_phone} from order journal action: ${result.message}`,
          "failed",
        );
      }
    } catch (err: any) {
      ...
      addAuditLog(
        "order",
        "resend_signup_bonus",
        `Error trying to resend welcome data bonus to recipient ${order.recipient_phone} from order journal: ${err.message || err}`,
        "failed",
      );
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-text-muted mt-1">
          Investigate recipient delivery feeds, requery pending carrier SME
          routes, and confirm statuses.
        </p>
      </div>
              ...
              {isLoading ? (
                ...
                Array.from({ length: 5 }).map((_, idx) => (
                  <tr key={idx} className="animate-pulse">
                    ...
                    <td className="px-6 py-4">
                      <div className="h-8 bg-slate-100 rounded-lg w-16 mx-auto" />
                    </td>
                  </tr>
                ))
              ) : orders.length === 0 ? (
                <tr>
                  ...
                  >
                    No VTU transactions matched your search criteria. Try
                    removing filters.
                  </td>
                </tr>
              ) : (
                orders.map((order) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-bold tracking-wide mr-1.5 ${
                            order.network === "MTN"
                      ...
                      <td className="px-6 py-4 text-right font-mono text-slate-500">
                        {order.cashback_amount > 0 ? (
                          <span className="text-purple-600">
                            {formatNaira(order.cashback_amount)}
                          </span>
                        ) : (
                          "—"
                        )}
                        ...
                        {(() => {
                          if (!order.smedata_ref && order.amount === 0) {
                            return (
                              <button
                                onClick={(e) =>
                                  handleResendBonusFromOrder(e, order)
                                }
              ...
              {Array.from({ length: totalPages }).map((_, idx) => {
                ...
                if (
                  ...
                  return pageNum === 2 || pageNum === totalPages - 1 ? (
                    <span
                      key={pageNum}
                      className="text-xs text-slate-450 px-1 font-semibold select-none"
                    >
                      ...
                    </span>
                  ) : null;
